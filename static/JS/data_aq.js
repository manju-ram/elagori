...
function sendData(outputNumber, data) {
  const url = "http://192.168.1.120:5000/dodata";
  
   const digitalOutputs = Array.from(toggleButtons).map((button) => (button.classList.contains("on") ? "1.00" : "0.00"));
   doArray.push(digitalOutputs);
   
   console.log("Digital Analog Last Data: "+ doArray[doArray.length-1]);

   let lastData=doArray[doArray.length-1];


    const body = { Digital_Output_Values: lastData.join(',') };
